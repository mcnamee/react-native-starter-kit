...
const styles = StyleSheet.create({
  ...
  button_text: {
    ...
    textAlign: 'center',
    fontSize: 15,
    lineHeight: parseInt(15 + (15 * 0.5), 10),
  ...
  buttonSml_text: {
    fontSize: 12,
    lineHeight: parseInt(12 + (12 * 0.5), 10),
  },
...
class Button extends Component {
  ...
  render = () => {
    const { text, type, onPress, size, disabled, color } = this.props;

    // Setup styles based on Props
    const buttonStyles = [styles.button];
    const textStyles = [AppStyles.baseText, styles.button_text];
